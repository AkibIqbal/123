...
#a 
#Construct life table with its different columns
lifetable<-new("lifetable",x=x_sample,lx=lx_sample, name="lifetable")
print(lifetable)

#b
#Create actuarial table with interest rate 3% in terms of monetary function
...
#f
DemoEX3<-exn(lifetable, 5,13, "complete")
c(DemoEX3)
...
#new_way
S <- 10000     # sum assured, payable at maturity
n <- 10        # full term of the policy, in years
i <- 0.10      # effective annual interest rate
t <- 5         # duration at which policy/paid-up value is required

sdd_n <- accumulatedValue(i = i, n = n, type = "due")
P <- S / sdd_n

sdd_t <- accumulatedValue(i = i, n = t, type = "due")
PolicyValue_t <- P * sdd_t

PaidUpValue <- PolicyValue_t * (1 + i)^(n -t)
PolicyValue_t
PaidUpValue
...
#6
#i
# Joint Life Calculation for individuals’ case
ages <- c(65, 62)
...
Year=c(0, 1, 5, 10, 15, 20, 25, 30, 40)
tpx2=tpx[Year+1]
tpy2=tpy[Year+1]
tpxy2=tpxy[Year+1]
tp_last2=tp_last[Year+1]

data.frame(Year,tpx=tpx2,tpy=tpy2,tpxy=tpxy2,tp_last=tp_last2)


#v
# Joint life annuity-due
axy <- sum(tpxy[1:(n + 1)] * v^(0:n))
cat(sprintf("\nJoint life annuity-due: %.4f\n", axy))
